...
func DownloadFromTarball(sftpClient *sftp.Client, remoteTarPath string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("%v failed to open remote file: %s", err, remoteTarPath)
	}
	defer remoteFile.Close()

	gzReader, err := gzip.NewReader(remoteFile)
	if err != nil {
		return 0, fmt.Errorf("%v failed to create new gzip reader", err)
	}
	...
	for {
		...
		if err != nil {
			return 0, fmt.Errorf("%v failed to iterate through tar reader", err)
		}

		targetPath := filepath.Join(cwd, header.Name)
		if !strings.HasPrefix(filepath.Clean(targetPath), cwd) {
			return 0, fmt.Errorf("%v illegal file path detected", err)
		}
		...
		case tar.TypeDir:
			...
			if err != nil {
				return 0, fmt.Errorf("%v failed to create directory %s", err, targetPath)
			}
		...
		case tar.TypeReg:
			...
			if err != nil {
				return 0, fmt.Errorf("%v failed to create file %s", err, targetPath)
			}
			_, err = io.Copy(outFile, tarReader)
			if err != nil {
				outFile.Close()
				return 0, fmt.Errorf("%v failed to write file %s", err, targetPath)
			}
			if !conf.Config.Silent {
				relPath, err := filepath.Rel(cwd, targetPath)
				if err != nil {
					return 0, fmt.Errorf("%v failed to get relative path for %s", err, targetPath)
				}
